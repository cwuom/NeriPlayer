...
- **一起听不是“同步一个进度条”这么简单**：
  ...
  Durable Object 缓存，听众可直接取得当前链接而无需等待房主再次响应；
...
- 🎧 **一起听**：
  ...
  提供邀请密钥，成员重连使用成员密钥。本地歌曲不能建房或替换房间当前曲目；直链共享开启
  时，Worker 只公开并缓存当前曲目的房主直链，听众可直接取得缓存，关闭共享会清空缓存。
...
- GitHub 同步正文以私有 Draft Release Asset 的原始二进制传输，仓库只提交小型 UTF-8 清单，
  不走 Contents API 的 Base64 正文或 1 MB 内联内容限制。
...
- 🪶 **省流模式**：`backup.bin` 直接写入原始 `GZIP(ProtoBuf)` 字节；读取侧仍兼容
  JSON 和历史 Base64 格式，关闭省流模式时使用 JSON。
- GitHub 同步将正文作为 Release Asset 二进制上传，以 UTF-8 清单提交协调版本，
  不受 Contents API 的 1MB 内联内容限制；WebDAV 没有 ETag/Last-Modified 时，
  只有远端 SHA-256 指纹未变化才会回退无条件写入，否则按并发冲突失败。
- 首次切换到 GitHub Release Asset 协议前，必须升级同一仓库的所有同步终端；
  只识别 Contents API `backup.*` 文件的旧客户端不能与该协议混用。
...
- GitHub/WebDAV 同步不是端到端加密；完整配置导出文件可能包含授权信息，
  请自行妥善保管。
- 省流同步写入使用原始 GZIP 字节；新版在迁移期间仅保留历史 Base64 的读取兼容。
