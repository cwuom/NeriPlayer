...
- **网易云音乐**：
  ...
  热门歌曲/专辑分页和艺术家关注；播放页评论支持热门（默认）、最新、推荐排序、被回复内容和楼中楼展开、
  长按复制，以及登录后的点赞、取消点赞、发评论和回复。发送失败保留草稿，网络结果不确定时不自动重发。
- **Bilibili**：
  Web 登录、二维码登录、视频搜索、创建收藏夹、订阅收藏夹、合集、
  收藏夹/合集列表搜索、分 P 转音频播放、下载；
  链接识别会保留选中的分 P，并能从 `season_id` 或视频详情恢复合集上下文；
  播放页评论支持热门（默认）和最新排序，以及登录后的点赞和取消点赞；最新列表及后续分页可能需要登录。
  评论楼号按当前列表顺序显示，支持楼中楼分页、长按复制，以及登录后的发评论和回复。
- **YouTube Music**：
...
- **歌词来源**：
  ...
  “设置 > 歌词设置”的“来源偏好”卡片可指定播放时优先使用的默认歌词来源
  （自动 / 网易云 / 酷狗 / QQ 音乐 / LRCLIB / AMLL TTML），并开关
  “优先使用逐词歌词”；默认跟随平台并优先逐词，逐词候选的排序可能与旧版不同。
  按歌名和歌手搜索指定来源时，还会校验歌曲时长；时长未知或不符则回退到自动歌词。
  在线来源歌曲先显示已缓存的歌词，再于后台尝试指定来源；匹配成功后替换，找不到时继续使用原有歌词。
  修改后会清理已缓存的歌词结果，
  让新偏好立即生效。
...
- [ ] 视频播放
- [x] 评论阅读、楼中楼、复制、发评、回复、排序与点赞（网易云 / Bilibili）
- [ ] 第三方平台播放、库页和账号能力持续扩展
