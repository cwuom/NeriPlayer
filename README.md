...
- **听感也能细调**：
  ...
  启动看门狗、前后台健康审计、动态传输扩缩容和卡死自动恢复也都补齐了。
- **下载链路已从“能下”进化到“能恢复”**：
...
- 🔌 **USB 独占播放**：
  ...
  跟随歌曲采样率时不会在 Native 独占里隐式改采样率，设备不支持时回到系统输出，
  避免低质量重采样带来的听感疲劳；
  播放启动卡住、Native 传输背压卡住或前后台切换异常时，会通过原地重配置、
  动态传输扩缩容和软恢复尽量拉回，必要时回退到系统播放避免整条链路卡死。
- 💾 **可配置流媒体缓存**：
...
- USB 独占目前面向 **UAC1.0** 与兼容 **UAC2.0 Type I PCM** 设备，支持设备选择、采样率/位深/缓冲策略、
  兼容性开关、32-bit PCM、PCM float 软件转换和后台缓冲区设置；Native 独占不会隐式改采样率，
  跟随歌曲时遇到设备不支持的采样率会回到系统输出；为了减少卡住，
  播放器层还包含启动看门狗、前后台健康审计、keep-alive 检查、原地重配置、
  动态传输扩缩容、Native 传输背压恢复，以及必要时的系统播放回退。
