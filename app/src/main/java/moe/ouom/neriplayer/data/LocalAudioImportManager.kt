            ...
            context.contentResolver.query(audioUri, projection, selection, null, null)?.use { cursor ->
                ...
                while (cursor.moveToNext()) {
                    ...
                    val fallbackSong = SongItem(
                        ...
                        name = cursor.getString(idxTitle).orEmpty().ifBlank { context.getString(R.string.local_files) },
                        artist = cursor.getString(idxArtist).orEmpty().ifBlank { context.getString(R.string.music_unknown_artist) },
                        album = cursor.getString(idxAlbum).orEmpty().ifBlank { context.getString(R.string.local_files) },
                        albumId = 0L,
                        durationMs = duration,
                        coverUrl = null,
                        mediaUri = contentUri.toString(),
                        originalName = cursor.getString(idxTitle).orEmpty().ifBlank { context.getString(R.string.local_files) },
                        originalArtist = cursor.getString(idxArtist).orEmpty().ifBlank { context.getString(R.string.music_unknown_artist) },
                        originalCoverUrl = null
                    )

                    songs += runCatching {
                        LocalMediaSupport.toSongItem(context, LocalMediaSupport.inspect(context, contentUri))
                    }.getOrElse {
