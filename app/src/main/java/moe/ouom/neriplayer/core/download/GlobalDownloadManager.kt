...
object GlobalDownloadManager {
    ...
    private fun resolvePendingDownloadRecoveryPlan(context: Context): PendingDownloadRecoveryPlan {
        ...
        recoveryCandidates.forEach { candidate ->
            ...
            when {
                ...
                songKey in externallyStoppedSongKeys -> Unit
                else -> {
                    resumableSongs += song
                }
            }
    ...
    private suspend fun finalizeCompletedDownload(
        ...
        val usesCoreCommitPath = existingMetadata?.downloadFinalized != true ||
    ...
    private suspend fun completeCoreDownloadAndEnqueueEnrichment(
        ...
        val operationId = existingMetadata?.operationId ?: UUID.randomUUID().toString()
    ...
    private suspend fun finalizeCompletedDownloadMetadata(
        ...
        return try {
            ...
            if (!shouldFinalizeDownloadedSidecars(
    ...
    private suspend fun handleCancelledCompletedDownload(
        ...
        val preserveCommittedAudio = shouldPreserveAudioAfterCancellation(
        ...
        if (preserveCommittedAudio) {
            NPLogger.d(
                ...
                "core commit 后收到迟到取消，保留完整音频: " +
                    "song=${song.name}, state=${storedMetadata?.artifactState}"
            )
        } else {
    ...
    internal suspend fun rollbackCancelledDownload(
        ...
        if (
            shouldPreserveAudioAfterCancellation(
    ...
    private suspend fun deleteDownloadedSongsOnIo(
        ...
        try {
            ...
            deletionResult.deletedSongs.forEach { song ->
                ...
                runCatching {
                    song.stableKey?.let { stableKey ->
                        ManagedLibraryItemRoomStore.delete(appContext, stableKey)
                    }
                }.onFailure { error ->
                    NPLogger.w(TAG, "删除 managed_library_item 预览失败: ${error.message}")
                }
            }
    ...
    private fun scheduleUserDownload(
        ...
        scope.launch {
            ...
            runCatching {
                ...
                NPLogger.w(TAG, "持久化下载 operation 失败，继续 OS 调度: ${error.message}")
            }
            ...
            if (schedule is DownloadExecutionSchedule.Rejected) {
                runCatching {
                    DownloadExecutionRoomStore.delete(appContext, operationId)
                }
                NPLogger.w(
                    TAG,
    ...
    internal suspend fun executeDownloadOperation(
        ...
        startDownloadConfirmed(
            context = appContext,
            song = song,
            cleanupBeforeStart = !preserveStaging,
    ...
    internal fun executionResultFor(songKey: String): DownloadExecutionResult {
        return when (taskStore.findTask(songKey)?.status) {
        ...
        val batchJob = scope.launch {
            ...
            try {
                ...
                pendingSongs.forEach { request ->
                    ...
                    val operationId = UUID.randomUUID().toString()
                    val executionRequest = DownloadExecutionRequest(
                        operationId = operationId,
                        song = song,
                    ...
                    runCatching {
                        ...
                        NPLogger.w(TAG, "持久化批量 operation 失败，继续 OS 调度: ${error.message}")
                    }
                    val schedule = DownloadExecutionHosts.default.schedule(
                        context = appContext,
                        request = executionRequest
                    ...
                    if (schedule is DownloadExecutionSchedule.Rejected) {
                        ...
                        NPLogger.w(
                            TAG,
                            "批量下载宿主调度失败: song=${song.name}, reason=${schedule.reason}"
                        )
                    } else {
                        NPLogger.d(
                            TAG,
    ...
    private fun requestAllDownloadTaskCancellation(): Job? {
        ...
        activeSongKeys.forEach { songKey ->
            DownloadExecutionHosts.default.cancelForSong(
                context = appContext,
                songKey = songKey
            )
